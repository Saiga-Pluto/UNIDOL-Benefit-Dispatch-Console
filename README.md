...
アップロードするCSVはA-F列まであれば使えます。送信するかどうかは、CSVではなく画面上のチェックボックスで選びます。

送信済みCSVを保存すると、以下の列が追加されます。
...
送信に成功した行はアプリ上で送信済みになり、`送信済みCSVを保存` からG列に送信済み状態を反映したCSVをダウンロードできます。保存ファイル名は、元ファイル名に日時を付けた形式です。

例:

```text
benefit-list_20260718-012345.csv
```

このCSVを次回アップロードすると、G列が`TRUE`の行は送信済みとして扱われ、送信対象に選べなくなります。
